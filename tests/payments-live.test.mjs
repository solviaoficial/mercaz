...
const { encrypt, createPayment, syncPayment, refundPayment, tokenFor } =
  await import('../server/payments.mjs');
run(
  'INSERT INTO users(id,name,email,password) VALUES(1,?,?,?)',
  'Comprador Teste',
  'buyer@example.com',
  passwordHash('SenhaTesteSomente2026!'),
);
run(
...
run(
  'INSERT INTO stores(id,user_id,name,description,mp_token,mp_refresh,mp_expires) VALUES(1,2,?,?,?,?,?)',
  'Loja Teste',
  'Descrição da loja.',
  encrypt('seller-token'),
  encrypt('refresh-token'),
  Date.now() + 3600000,
);
run(
  "INSERT INTO products(id,store_id,title,description,category,price,stock,images) VALUES(1,1,'Produto','Descrição do produto','Casa e decoração',10000,2,'[]')",
);
const id = 'MCZ-' + randomUUID();
run(
  "INSERT INTO orders(id,user_id,store_id,subtotal,shipping,fee,total,address,prep_days,shipping_days,demo,checkout_key) VALUES(?,1,1,10000,1000,500,11000,'{}',2,5,0,?)",
  id,
  randomUUID(),
);
run(
  "INSERT INTO order_items(order_id,product_id,title,image,variant,qty,price) VALUES(?,1,'Produto','/assets/test.webp','Único',1,10000)",
  id,
);
...
test('Pix marketplace: corpo, split, OAuth, timeout, confirmação e reembolso', async (t) => {
  await t.test(
    'timeout conserva payload criptografado para recuperar sem nova cobrança',
    async () => {
      const o = one('SELECT * FROM orders WHERE id=?', id);
      await assert.rejects(() =>
        createPayment(
          o,
          { name: 'Comprador Teste', email: 'buyer@example.com' },
          '12345678909',
        ),
      );
      const payload = one(
      ...
      assert.equal(
        one('SELECT payment_id FROM orders WHERE id=?', id).payment_id,
        null,
      );
    },
  );
  await t.test('nova tentativa usa chave e corpo idênticos', async () => {
    ...
    await createPayment(
      o,
      { name: 'Nome diferente', email: 'outro@example.com' },
      '00000000000',
    );
    ...
    assert.equal(
      one('SELECT payload FROM payment_requests WHERE order_id=?', id),
      undefined,
    );
  });
  await t.test('consulta ao provedor confirma pagamento uma vez', async () => {
    providerStatus = 'approved';
    await syncPayment(one('SELECT * FROM orders WHERE id=?', id));
    await syncPayment(one('SELECT * FROM orders WHERE id=?', id));
    assert.equal(
      one('SELECT status FROM orders WHERE id=?', id).status,
      'confirmed',
    );
    assert.equal(
      one(
        "SELECT COUNT(*) n FROM events WHERE order_id=? AND status='confirmed'",
        id,
      ).n,
      1,
    );
  ...
  await t.test('OAuth renova e persiste os tokens criptografados', async () => {
    run('UPDATE stores SET mp_expires=0 WHERE id=1');
    assert.equal(await tokenFor(1), 'renewed-token');
    const s = one('SELECT * FROM stores WHERE id=1');
    assert.notEqual(s.mp_token, 'renewed-token');
    assert.ok(s.mp_expires > Date.now());
    assert.equal(
      JSON.parse(requests.at(-1).opts.body).grant_type,
      'refresh_token',
    );
  });
  await t.test(
    'reembolso usa chave estável e reconcilia com o provedor',
    async () => {
      await refundPayment(one('SELECT * FROM orders WHERE id=?', id));
      assert.equal(
        one('SELECT status FROM orders WHERE id=?', id).status,
        'refunded',
      );
