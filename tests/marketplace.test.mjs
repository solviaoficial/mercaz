...
const { applyPayment, validSignature, release } =
  await import('../server/payments.mjs').then(async (m) => ({
    ...m,
...
test('Autenticação, isolamento e perfil', async (t) => {
  await t.test('rotas privadas exigem conta', async () =>
    assert.equal((await call('/orders')).code, 401),
  );
  ...
  await t.test('cadastro, favorito e endereço persistem', async () => {
    ...
    assert.equal(
      (await call('/favorites', { kind: 'product', item_id: 47 }, newUser))
        .code,
      200,
    );
  ...
  await t.test('vendedor não edita produto alheio', async () =>
    assert.equal(
      (await call('/products/47/stock', { stock: 50 }, other, 'PATCH')).code,
      404,
    ),
  );
});
test('Compra transacional e estoque', async (t) => {
  await t.test(
    'preço do cliente é ignorado e pedidos são separados por loja',
    async () => {
      ...
      assert.equal(one('SELECT stock FROM products WHERE id=47').stock, 16);
      const repeat = await call(
        '/checkout',
        { key, address, accepted: true, items: [] },
        buyer,
      );
  ...
  await t.test('estoque insuficiente desfaz todos os grupos', async () => {
    const before = one('SELECT stock FROM products WHERE id=101').stock;
    const r = await call(
      '/checkout',
      {
        key: randomUUID(),
    ...
    assert.equal(one('SELECT stock FROM products WHERE id=101').stock, before);
  });
  await t.test(
    'quantidades negativas, variações falsas e itens duplicados falham',
    async () => {
      for (const items of [
        ...
        assert.equal(
          (
            await call(
              '/checkout',
              { key: randomUUID(), address, accepted: true, items },
              buyer,
            )
          ).code,
          400,
  ...
  await t.test('avaliação antes da entrega é bloqueada', async () =>
    assert.equal(
      (
        await call(
          '/reviews',
  ...
  await t.test(
    'pagamento idempotente e fluxo completo até entrega',
    async () => {
      assert.equal(
        (await call('/orders/' + orderId + '/simulate', {}, buyer)).code,
        200,
      );
      await call('/orders/' + orderId + '/simulate', {}, buyer);
      assert.equal(
        all(
          "SELECT * FROM events WHERE order_id=? AND status='confirmed'",
          orderId,
        ).length,
        1,
      );
      for (const state of ['preparing', 'ready', 'shipped', 'transit'])
        assert.equal(
          (
            await call(
              '/seller/orders/' + orderId + '/status',
              { status: state, tracking: 'Correios TESTE12345' },
              seller,
            )
          ).code,
          200,
      ...
      assert.equal(
        (await call('/orders/' + orderId + '/delivered', {}, buyer)).code,
        200,
      );
      assert.equal(
        one('SELECT status FROM orders WHERE id=?', orderId).status,
        'delivered',
      );
    },
  );
  await t.test('avaliação verificada, reação e moderação', async () => {
    ...
    assert.equal(
      (
        await call(
          '/reviews',
          { order_id: orderId, product_id: 47, rating: 4, text: 'Repetida' },
          buyer,
        )
      ).code,
      409,
    ...
    assert.equal(
      (await call('/reviews/' + r.data.id + '/helpful', {}, newUser)).code,
      200,
    );
    assert.equal(
      (
        await call(
          '/reviews/' + r.data.id + '/report',
          { reason: 'Teste de análise de conteúdo' },
          newUser,
        )
      ).code,
      201,
    ...
    assert.equal(
      (await call('/admin/reports/' + reports[0].id, { hide: false }, buyer))
        .code,
      200,
    );
  });
  await t.test('suporte conversa entre comprador e loja', async () => {
    ...
    assert.equal(
      (await call('/tickets/' + r.data.id, null, buyer)).data.messages.length,
      2,
    );
  });
  await t.test('reembolso devolve estoque uma única vez', async () => {
    assert.equal(
      (await call('/seller/orders/' + orderId + '/refund', {}, seller)).code,
      200,
    );
    assert.equal(one('SELECT stock FROM products WHERE id=47').stock, 18);
    release(orderId, 'refunded');
    assert.equal(one('SELECT stock FROM products WHERE id=47').stock, 18);
    assert.equal(
      (await call('/seller/orders/' + orderId + '/refund', {}, seller)).code,
      400,
...
test('Conteúdo, uploads e assistente', async (t) => {
  ...
  await t.test(
    'upload autorizado e criação de anúncio persistente',
    async () => {
      const f = new FormData();
      f.append(
        'file',
  ...
  await t.test('pergunta e resposta vinculadas à loja correta', async () => {
    await call(
      '/questions',
      { product_id: 47, question: 'Qual é o tamanho deste produto?' },
      buyer,
    );
    const id = one('SELECT id FROM questions ORDER BY id DESC').id;
    assert.equal(
      (
        await call(
          '/questions/' + id + '/answer',
          { answer: 'Vamos conferir as medidas.' },
          other,
        )
      ).code,
      404,
...
test('Contrato de pagamentos e assinaturas', async (t) => {
  ...
  await t.test(
    'aprovação exige valor, moeda e referência correspondentes',
    async () => {
      const o = one("SELECT * FROM orders WHERE status='pending' LIMIT 1");
      assert.ok(o);
      ...
      assert.equal(
        one('SELECT status FROM orders WHERE id=?', o.id).status,
        'pending',
      );
