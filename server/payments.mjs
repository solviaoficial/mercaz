...
export async function mp(path, token, body, method = 'POST', idempotency) {
  ...
  if (!r.ok) {
    const e = new Error(
      'O Mercado Pago não concluiu a operação. Tente novamente ou verifique a conexão da loja.',
    );
    e.status = 502;
    throw e;
  }
...
export async function tokenFor(storeId) {
  const s = one('SELECT * FROM stores WHERE id=?', storeId);
  if (!s?.mp_token)
    throw Object.assign(
      new Error('Esta loja ainda precisa conectar o Mercado Pago.'),
      { status: 409 },
    );
  if (s.mp_expires > Date.now() + 60000) return decrypt(s.mp_token);
  if (refreshing.has(storeId)) return refreshing.get(storeId);
  const task = (async () => {
    ...
    run(
      'UPDATE stores SET mp_token=?,mp_refresh=?,mp_expires=? WHERE id=?',
...
export async function createPayment(o, user, cpf) {
  ...
  let saved = one(
    'SELECT payload FROM payment_requests WHERE order_id=?',
    o.id,
  );
  if (!saved) {
    const payload = {
      ...
      date_of_expiration: new Date(
        Date.parse(o.created + 'Z') + 30 * 60000,
      ).toISOString(),
    ...
    run(
      'INSERT OR IGNORE INTO payment_requests VALUES(?,?)',
      o.id,
      encrypt(JSON.stringify(payload)),
    );
    saved = one('SELECT payload FROM payment_requests WHERE order_id=?', o.id);
  }
  const d = await mp(
    '/v1/payments',
    token,
    JSON.parse(decrypt(saved.payload)),
    'POST',
    o.id,
  );
  transaction(() => {
    run(
      'UPDATE orders SET payment_id=?,payment_data=? WHERE id=?',
      String(d.id),
      JSON.stringify(d.point_of_interaction?.transaction_data || {}),
      o.id,
    );
    run('DELETE FROM payment_requests WHERE order_id=?', o.id);
  });
...
export async function cancelPayment(o) {
  ...
  if (!o.payment_id)
    throw Object.assign(
      new Error(
        'Verifique ou gere o Pix antes de cancelar; uma tentativa anterior pode estar sendo processada.',
      ),
      { status: 409 },
    );
  await syncPayment(o);
  if (one('SELECT status FROM orders WHERE id=?', o.id).status !== 'pending')
    throw Object.assign(
      new Error('O pagamento já foi atualizado. Recarregue o pedido.'),
      { status: 409 },
    );
  const d = await mp(
    '/v1/payments/' + o.payment_id,
...
export async function refundPayment(o) {
  if (!demo) {
    await mp(
      `/v1/payments/${o.payment_id}/refunds`,
      await tokenFor(o.store_id),
      {},
      'POST',
      'refund-' + o.id,
...
export function validSignature(req) {
  ...
  const id = String(req.query['data.id'] || '').toLowerCase(),
  ...
  if (Math.abs(Date.now() - millis) > 10 * 60000) return false;
...
export async function reconcile() {
  ...
  try {
    for (const o of all(
      "SELECT * FROM orders WHERE status='pending' OR (payment_id IS NOT NULL AND status NOT IN ('cancelled','refunded'))",
    )) {
      try {
        if (!o.payment_id && !demo) {
          ...
          if (saved) {
            ...
            await createPayment(
              o,
              { email: payload.payer.email, name: payload.payer.first_name },
              payload.payer.identification.number,
            );
          } else {
            const age = Date.now() - Date.parse(o.created + 'Z');
            if (age > 40 * 60000) {
              release(o.id, 'cancelled');
              continue;
            }
        ...
        if (
          current.status === 'pending' &&
          Date.now() - Date.parse(o.created + 'Z') > 40 * 60000
        )
          await cancelPayment(one('SELECT * FROM orders WHERE id=?', o.id));
      } catch (e) {
        console.error('Reconciliação pendente:', o.id, e.message);
      }
    }
    run('DELETE FROM sessions WHERE expires<?', Date.now());
    run('DELETE FROM oauth WHERE expires<?', Date.now());
  } finally {
    reconciling = false;
  }
}
