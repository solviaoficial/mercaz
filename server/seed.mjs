...
export function seed() {
  if (process.env.SEED_DEMO !== 'true' || one('SELECT id FROM users LIMIT 1'))
    return;
  const pass = process.env.DEMO_PASSWORD;
  if (!pass || pass.length < 12)
    throw Error(
      'Defina DEMO_PASSWORD com pelo menos 12 caracteres para criar o catálogo de demonstração.',
    );
  transaction(() => {
    run(
      'INSERT INTO users(id,name,email,password) VALUES(1,?,?,?)',
      'Visitante Demo',
      'comprador@mercaz.local',
      passwordHash(pass),
    );
    const shops = [
      [
    ...
    shops.forEach(([name, description], i) => {
      run(
        'INSERT INTO users(id,name,email,password) VALUES(?,?,?,?)',
        i + 2,
        name,
        `vendedor${i + 1}@mercaz.local`,
        passwordHash(pass),
      );
      run(
        'INSERT INTO stores(id,user_id,name,description,prep_days,shipping,shipping_days) VALUES(?,?,?,?,?,?,?)',
        i + 1,
        i + 2,
        name,
        description,
    ...
    products.forEach(([id, store, title, category, price, stock, v]) =>
      run(
        'INSERT INTO products(id,store_id,title,description,category,price,stock,images,variants) VALUES(?,?,?,?,?,?,?,?,?)',
        id,
        store,
        title,
        `${title}. Item do catálogo de demonstração da Mercaz. Imagens ilustrativas. Para uma venda real, o vendedor deve preencher material, medidas, conteúdo da embalagem, garantia e condições específicas do produto.`,
        category,
        price,
        stock,
        JSON.stringify([`/assets/product-${id}.webp`]),
        JSON.stringify([v]),
      ),
    );
  });
