...
export function notify(user, text, href) {
  run(
    'INSERT INTO notifications(user_id,text,href) VALUES(?,?,?)',
    user,
    text,
    href,
  );
...
export function status(id, state) {
  run(
    'UPDATE orders SET status=?,updated=CURRENT_TIMESTAMP WHERE id=?',
    state,
    id,
  );
  run('INSERT INTO events(order_id,status) VALUES(?,?)', id, state);
  const o = one('SELECT * FROM orders WHERE id=?', id);
  notify(
    o.user_id,
    'Seu pedido ' + id.slice(-6) + ' foi atualizado.',
    '/pedidos/' + id,
  );
}
export function release(id, state) {
  transaction(() => {
    const o = one('SELECT * FROM orders WHERE id=?', id);
    if (['cancelled', 'refunded'].includes(o.status)) return;
    for (const i of all('SELECT * FROM order_items WHERE order_id=?', id))
      run('UPDATE products SET stock=stock+? WHERE id=?', i.qty, i.product_id);
    status(id, state);
  });
}
...
export const publicUser = (u) =>
  u
    ? {
        ...
        seller: !!one('SELECT id FROM stores WHERE user_id=?', u.id),
      }
    : null;
